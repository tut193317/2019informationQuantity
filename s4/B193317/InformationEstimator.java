...
public class InformationEstimator implements InformationEstimatorInterface{
    ...
    double iq(int freq) {
	if( mySpace.length > 0)	return  - Math.log10((double) freq / (double) mySpace.length)/ Math.log10((double) 2.0);
	...
	*/
	...
	if(mySpace == null) return Double.MAX_VALUE;
	  for(int j = 0; j < myTarget.length; j++){
	       	...
	       	infoQuant[0][j] = Math.min(Double.MAX_VALUE,iq(myFrequencer.frequency()));
       	}
